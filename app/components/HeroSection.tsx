...
interface HeroSlide { eyebrow:string; titleParts:{text:string;highlight?:boolean}[]; subtitle:string; }

const SLIDES: HeroSlide[] = [
  { eyebrow: "Next-Gen Digital Solutions", titleParts: [{ text: "We Build " }, { text: "Intelligent", highlight: true }, { text: " Digital Experiences" }], subtitle: "From AI-powered platforms to stunning web apps. We engineer scalable, future-ready technology that drives real business growth." },
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for(let i=0;i<ps.length;i++){for(let j=i+1;j<ps.length;j++){
        const a=ps[i];const b=ps[j];const dx=a.x-b.x;const dy=a.y-b.y;const d=Math.sqrt(dx*dx+dy*dy);
      ...
      for(const p of ps){const dx=p.x-mx;const dy=p.y-my;const d=Math.sqrt(dx*dx+dy*dy);
  ...
  return (
    <>
      ...
      <section className="hz" aria-label="Zyvionix Hero" style={{position:"relative",width:"100vw",minHeight:isMobile?"100vh":"100svh",height:isMobile?"auto":"100svh",overflow:"hidden",marginLeft:"calc(50% - 50vw)",marginRight:"calc(50% - 50vw)",background:"#0d0814",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",paddingTop:isMobile?"80px":"0",paddingBottom:isMobile?"20px":"0"}}>
        ...
        <div aria-hidden="true" style={{position:"absolute",top:"8%",left:"4%",width:isMobile?180:360,height:isMobile?180:360,borderRadius:"50%",background:"radial-gradient(circle,rgba(124,58,237,0.24) 0%,transparent 70%)",filter:"blur(65px)",animation:"hzF1 14s ease-in-out infinite",zIndex:1,pointerEvents:"none"}}/>
        <div aria-hidden="true" style={{position:"absolute",bottom:"12%",right:"5%",width:isMobile?160:300,height:isMobile?160:300,borderRadius:"50%",background:"radial-gradient(circle,rgba(192,38,211,0.2) 0%,transparent 70%)",filter:"blur(55px)",animation:"hzF2 17s ease-in-out infinite",zIndex:1,pointerEvents:"none"}}/>
        ...
        <div style={{position:"relative",zIndex:10,display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center",padding:"0 clamp(1rem,4vw,5rem)",paddingTop:isMobile?"20px":"clamp(64px,10vh,100px)",maxWidth:920,width:"100%"}}>
          <div key={"b"+slide} style={{display:"inline-flex",alignItems:"center",gap:"0.55rem",marginBottom:isMobile?"1rem":"1.8rem",padding:"0.35rem 0.9rem",borderRadius:9999,border:"1px solid rgba(139,92,246,0.42)",background:"rgba(124,58,237,0.1)",backdropFilter:"blur(14px)",animation:"hzBP 0.4s cubic-bezier(0.34,1.56,0.64,1) both"}}>
            ...
            <span style={{fontSize:isMobile?"0.58rem":"0.64rem",fontWeight:800,letterSpacing:"0.2em",textTransform:"uppercase",color:"#c4b5fd",whiteSpace:"nowrap"}}>{cur.eyebrow}</span>
          </div>
          <h1 className={"hz-h1 "+(textVisible?"hzte":"hztx")} style={{fontSize:"clamp(1.8rem,6.5vw,5.2rem)",fontWeight:900,lineHeight:1.1,color:"#ffffff",margin:"0 0 1.2rem",letterSpacing:"-0.02em"}}>
            {cur.titleParts.map((p,i)=>p.highlight?<span key={i} className="hzsh">{p.text}</span>:<span key={i}>{p.text}</span>)}
          </h1>
          <p className={textVisible?"hzte":"hztx"} style={{fontSize:"clamp(0.9rem,1.8vw,1.2rem)",color:"rgba(255,255,255,0.7)",fontWeight:300,lineHeight:1.6,marginBottom:isMobile?"1.5rem":"2.8rem",maxWidth:680,transitionDelay:"0.05s"}}>{cur.subtitle}</p>
          <div style={{display:"flex",flexWrap:"wrap",gap:"0.75rem",justifyContent:"center",alignItems:"center",marginBottom:isMobile?"1.5rem":"2.5rem"}}>
            <Link href="/services" id="hero-cta-services" className="hzp" style={{padding:isMobile?"0.75rem 1.4rem":"0.9rem 2.1rem",fontSize:isMobile?"0.75rem":"0.82rem"}}>
              Explore Services
